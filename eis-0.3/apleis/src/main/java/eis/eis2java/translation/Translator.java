...
public class Translator {
  ...
  public <T> Parameter[] translate2Parameter(T o) throws TranslationException {
    ...
    if (rawTranslator == null) {
      throw new TranslationException("No translator found for class " + o.getClass().getName());
    }
   ...
   * @param parameterClass The class to which the parameter should be
   *        translated.
  ...
  public <T> T translate2Java(Parameter parameter, Class<T> parameterClass)
    ...
    if (rawTranslator == null) {
      throw new TranslationException("No translator found for class " + parameterClass.getName());
    }
    ...
    public Character translate(Parameter parameter) throws TranslationException {
      ...
      if (value.length() > 1) {
        throw new TranslationException(
            "A single character was expected instead a string of length " + value.length()
                + " was given. Contents: " + value);
      }
  ...
  private static class CollectionTranslator<T> implements Java2Parameter<AbstractCollection<T>> {
    ...
    public Class<? extends AbstractCollection<T>> translatesFrom() {
      return (Class<? extends AbstractCollection<T>>) AbstractCollection.class;
    }
  }
